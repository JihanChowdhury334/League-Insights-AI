...
# FIX SSL TIMEOUT / STALE CONNECTION ERRORS
app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
    "pool_pre_ping": True,     # Always verify connection
    "pool_recycle": 280,       # Recycle before DB closes idle SSL (~300 sec)
    "pool_size": 5,
    "max_overflow": 10
}
...
async def process_timelines():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            # Step 5: Insert into database in batches using execute_values
            if results:
                batch_size = 50  # Define the batch size
                print(f"[DB] Resetting database connection before inserts")
                reset_db_connection()
                ...
                try:
                    for i in range(0, len(results), batch_size):
                        ...
                        # Build values tuple for execute_values
                        values = [
                            (
                                s["match_id"],
                                s["puuid"],
                                s["early_dominance_score"],
                                s["midgame_swing_score"],
                                s["consistency_score"],
                                s["level_6_timestamp"],
                                s["level_11_timestamp"],
                                s["level_16_timestamp"],
                                s["biggest_spike"],
                                s["biggest_throw"],
                                s["roam_score"],
                                json.dumps(s["kill_positions"]),
                                json.dumps(s["objective_presence"]),
                                s["comeback_type"],
                                s["duration"]
                            )
                            for s in batch
                        ...
                        while retries > 0:
                            try:
                                ...
                                try:
                                    with conn.cursor() as cursor:
                                        execute_values(
                                            cursor,
                                            """
                                            INSERT INTO match_timeline_summary (
                                                match_id, puuid,
                                                early_dominance_score, midgame_swing_score, consistency_score,
                                                level_6_timestamp, level_11_timestamp, level_16_timestamp,
                                                biggest_spike, biggest_throw, roam_score,
                                                kill_positions, objective_presence,
                                                comeback_type, duration
                                            ) VALUES %s
                                            """,
                                            values
                                        )
                                ...
                                print(f"[DB] Inserted batch {i // batch_size + 1}/{(len(results) + batch_size - 1) // batch_size}")
                                break  # Exit retry loop on success
                            except psycopg2.OperationalError as e:
                                retries -= 1
                                print(f"[DB] Database operation failed. Retries left: {retries}. Error: {e}")
                                if retries == 0:
                                    raise
                                await asyncio.sleep(2)  # Wait before retrying

                    print(f"[DB] Successfully inserted {len(results)} timeline summaries into the database.")
                    reset_db_connection()

                except Exception as e:
                    print(f"[DB] ERROR: Failed to insert timeline summaries: {e}")
